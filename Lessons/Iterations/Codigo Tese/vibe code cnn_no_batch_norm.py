"""
CNN Implementation for Signal Processing with Multiple Signals (No Batch Normalization)
=============================================================
...
This version does not use batch normalization in the neural network.
"""
...
# --- MAIN EXECUTION ---
print("\n" + "="*50)
print("CNN IMPLEMENTATION WITHOUT BATCH NORMALIZATION")
print("="*50 + "\n")
...
os.makedirs('output', exist_ok=True)
preprocessed_path = 'output/preprocessed_data_no_batch_norm.csv'
...
epsilon = 1e-10
# Convert back to original scale for percentage calculations
y_test_orig_flat = y_test_original.reshape(-1)
y_pred_orig_flat = y_pred_original.reshape(-1)
mape = np.mean(np.abs((y_test_orig_flat - y_pred_orig_flat) / (np.abs(y_test_orig_flat) + epsilon))) * 100

# Calculate accuracy-like metric (percentage of predictions within a certain threshold)
threshold = 0.1  # 10% of the true value
within_threshold = np.mean(np.abs((y_test_orig_flat - y_pred_orig_flat) / (np.abs(y_test_orig_flat) + epsilon)) < threshold) * 100
...
os.makedirs('output', exist_ok=True)
cnn_processed_path = 'output/cnn_processed_data_no_batch_norm.csv'
...
plt.tight_layout()
plt.savefig('output/cnn_results_no_batch_norm.png')
print("Results visualization saved to output/cnn_results_no_batch_norm.png")
plt.show()

# Step 15: Print model summary
print("\nSummary of CNN Implementation (No Batch Normalization):")
