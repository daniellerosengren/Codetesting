# -*- coding: utf-8 -*-
"""
This code reproduce the results of Fig. 6 based on the experimental data
"""
import numpy as np
import torch
import time
...
freq_interval = 0.01             # frequenncy difference intervals
freq_init = 1.6                 # initial frequency difference
freq_num = 1                    # number of frequency difference points
average_num = 1                 # in case one is interested in averaging

freq_diff =np.zeros((1, freq_num))
freq_diff[0] =  np.arange(freq_init, freq_init + (1)*freq_interval + 0.0001, freq_interval)
...
for df in range(freq_num):
    for k in range(average_num):
        ...
        Ndata = int(880*25000)        
        data01n = np.reshape(np.transpose(np.genfromtxt('Data1.txt')),(1,Ndata))
        data02n = np.reshape(np.transpose(np.genfromtxt('Daat2.txt')),(1, Ndata))
        data01n[data01n > 0] = 1
        data02n[data02n > 0] = 1
