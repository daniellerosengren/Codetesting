...
# Removed pywt import as it's not available
...
def extract_fft_features(signal_data, n_components=10):
    """Extract frequency domain features using Fast Fourier Transform."""
    n_samples = signal_data.shape[0]
    n_timesteps = signal_data.shape[1]
    
    # Compute FFT for each signal
    fft_features = np.zeros((n_samples, n_components))
    
    for i in range(n_samples):
        # Compute FFT
        yf = fft(signal_data[i])
        # Get absolute values of the first n_components frequencies
        fft_features[i] = np.abs(yf[:n_components])
    
    return fft_features

def extract_simple_scale_features(signal_data, n_scales=3):
    """Extract simple multi-scale features without using wavelets."""
    n_samples = signal_data.shape[0]
    
    # Initialize feature array - 2 features per scale (mean, std)
    scale_features = np.zeros((n_samples, n_scales * 2))
    ...
    for i in range(n_samples):
        ...
        for j in range(n_scales):
            # Simple scaling - take every 2^j points
            scale = 2**j
            if scale < len(signal):
                # Subsample the signal
                subsampled = signal[::scale]
                # Extract features
                scale_features[i, j*2] = np.mean(subsampled)
                scale_features[i, j*2+1] = np.std(subsampled)
            else:
                # Fallback for large scales
                scale_features[i, j*2] = np.mean(signal)
                scale_features[i, j*2+1] = np.std(signal)
...
def extract_statistical_features(signal_data):
    ...
    # We'll extract mean, std, min, max, range, skewness, kurtosis
    stat_features = np.zeros((n_samples, 7))
    
    for i in range(n_samples):
        signal = signal_data[i]
        stat_features[i, 0] = np.mean(signal)
        stat_features[i, 1] = np.std(signal)
        stat_features[i, 2] = np.min(signal)
        stat_features[i, 3] = np.max(signal)
        stat_features[i, 4] = np.max(signal) - np.min(signal)  # range
        
        # Skewness
        m3 = np.mean((signal - np.mean(signal))**3)
        if np.std(signal) > 0:
            stat_features[i, 5] = m3 / (np.std(signal)**3)
        else:
            stat_features[i, 5] = 0
        ...
        if np.std(signal) > 0:
            stat_features[i, 6] = m4 / (np.std(signal)**4) - 3  # excess kurtosis
        else:
            stat_features[i, 6] = 0
...
def preprocess_data(X, y):
    ...
    fft_features = extract_fft_features(y, n_components=10)
    
    print("Extracting multi-scale features...")
    scale_features = extract_simple_scale_features(y, n_scales=3)
    ...
    combined_features = np.hstack((X, fft_features, scale_features, stat_features))
...
print("   - FFT features: Frequency domain information")
print("   - Multi-scale features: Information at different time scales")
print("   - Statistical features: Time domain characteristics")
...
print("\nThis model now takes enhanced features as input and predicts the probability signal plus noise (y).")
