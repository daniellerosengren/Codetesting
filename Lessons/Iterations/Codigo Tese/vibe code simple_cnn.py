...
# --- DATA AUGMENTATION ---
# This function is kept for reference but is no longer used with our new data format
...
# We need to modify the augmentation function to work with our new data format
...
# We don't need to normalize the time vectors, but we should normalize the signals
...
X_train, X_test, y_train, y_test, params_train, params_test = train_test_split(
    X, y_scaled, params, test_size=0.2, random_state=42)

# Further split training data to get validation set
X_train, X_val, y_train, y_val, params_train, params_val = train_test_split(
    X_train, y_train, params_train, test_size=0.1, random_state=42)

# Create neural network with CNN architecture
print("Creating CNN neural network...")
model = SimpleNN()

# Modify the network to predict signals from time
# First, we need to reshape the time vectors to match the expected input shape
...
model.add(Dense(units=n_timesteps))  # Output: signal values at each timestep
...
for i in range(4):
    ...
    # Get original signal (inverse transform to get back to original scale)
    original_signal = scaler.inverse_transform(y_test[i].reshape(1, -1)).flatten()
    predicted_signal = scaler.inverse_transform(y_pred[i].reshape(1, -1)).flatten()
    
    # Plot the signals
    plt.plot(X_test[i], original_signal, 'b-', label='True Signal')
    plt.plot(X_test[i], predicted_signal, 'r--', label='Predicted Signal')
...
print(f"   - 64 units → {n_timesteps} output neurons (signal values)")
